...
namespace CalceranosInvaders
{
	...
	public class Invaders
	{
		...
		public Invaders(Random r,Panel invadersPanel,Panel invadersPanel1)
		{
		...
		public int CheckHit(int x,int y)
		{
			...
			if (_isInvaderAlive[i,j])
			{
                ...
                var controls = _invadersPanel.Controls;
                var control = controls["lb_Invader" + row + column];

				if (x>= control.Left+_x && x<= control.Left+_x+ control.Width && y>= control.Top+_y && y<= control.Top+_y+ control.Height)
				{
