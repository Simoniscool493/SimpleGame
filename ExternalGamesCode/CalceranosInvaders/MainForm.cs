...
namespace CalceranosInvaders
{
	public partial class MainForm : Form
	{
        ...
        public int Score => _points;
        public bool IsGameOver;
        public bool GetBufferedInput;
        public static bool IsHeadless;
        static bool normalKeyInput = false;
        public List<int> inputBuffer = new List<int>();
        public static bool IsPiped;
        public NamedPipeClientStream stream;
        public static int InvadersLevel;
		...
		public MainForm(bool isHeadless,bool isPiped)
		{
            ...
            {
                throw new Exception("Taking input from keyboard in headless mode :/");
            }

            if(isPiped)
            {
                stream = new NamedPipeClientStream("SpaceInvadersPipe");
                stream.Connect();


                new Task(InputLoop).Start();
            }
        }
        ...
        {
            Thread.Sleep(500);
            var firstStatus = GetStatus();
            SendStatusInPipe(firstStatus);
            ...
            while (!IsGameOver)
            {
                ...
                inputBuffer.Add(input[0]);

                InvokeUI(Tick); 

                var status = GetStatus();
                SendStatusInPipe(status);
            }
        }
        ...
        public void SendStatusInPipe(int[] status)
        {
            byte[] toSend = new byte[status.Length];
            
            for(int i=0;i<status.Length;i++)
            {
                toSend[i] = (byte)(status[i]);
            }

            stream.Write(toSend, 0, status.Length);
        }
        ...
        public int[] GetStatus()
        {
            if(IsGameOver)
            {
                return new int[] { 254, Score % 255, Score/ 255 };
            }


            int isBulletAboveMe = 0;
            if (_invadersBullet.X >= _ship.X && _invadersBullet.X <= _ship.X + this.lb_Ship.Width) { isBulletAboveMe = 1; }

            int isBullet2AboveMe = 0;
            if (_invadersBullet1.X >= _ship.X && _invadersBullet1.X <= _ship.X + this.lb_Ship.Width) { isBullet2AboveMe = 1; }


            int isBulletOut = 0;
            if (_shipBullet.IsActive) { isBulletOut = 1; }

            return new int[] { isBulletAboveMe, isBullet2AboveMe, isBulletOut, InvadersLevel};
        }
        ...
        void Tm_FullTick(object sender, EventArgs e)
        {
            Tick();
        }

        private void KeysTick()
        {
            ...
            else
            {
                ...
                else if ((!_shipBullet.IsActive) && input == 3)
                {
                    if (_ship.IsActive)
                    {
                        _shipBullet.Start(_ship.X + 15, _ship.Y - lb_Bullet.Height, this.Height, 0, -6);
        ...
        public void InitializeObjects()
        {
            ...
            _invaders = new Invaders(_random,this.pn_InvadersContainer, this.panel1);
        ...
        {
            var count1 = _bunkersGrid.GetLength(0);
            var count2 = _bunkersGrid.GetLength(1);
            var count3 = _bunkersGrid.GetLength(2);
            ...
            for (int i = -2; i <= 2; i = i + 1)
            {
                for (int j = -2; j <= 2; j = j + 1)
                {
                    ...
                    if (num1 >= count2 || num2 >= count3 || num1 < 0 || num2 < 0)
                    {
                        continue;
                    }

                    if (bunkerCounter < 0 || bunkerCounter >= count1)
                    {
                        Console.WriteLine();
                    }

                    bunkerFound = _bunkersGrid[bunkerCounter, num1, num2];
            ...
            if (bunkerFound)
            {
                ...
                for (int i = -3; i <= 3; i = i + 1)
                {
                    for (int j = -4; j <= 4; j = j + 1)
                    {
                        ...
                        _bunkersGrid[bunkerCounter, num1, num2] = false;
                    }
